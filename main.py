...
class CreativeHour:
    ...
    def already_in_group(self, message, markup):
        ...
        markup.add(types.InlineKeyboardButton("Left group", 
                                            callback_data="markdell"))
        markup.add(types.InlineKeyboardButton("Try again", 
                                            callback_data="tryagain"))
        self.try_send_message(message.from_user.id, ALREADY_IN_GROUP_TEXT(), 
                            reply_markup=markup)
        
    def upgrade_room_info_time(self, id_chat):
        min_time, max_time = self.data_base.get_time_active_chat_users(id_chat)
        self.data_base.update_room_info_time(id_chat, min_time, 
                                            'min_start_time')
        self.data_base.update_room_info_time(id_chat, max_time, 
                                            'max_end_time')
        return None
    ...
    def pin_first_message(self, chat_id):
            first_message = self.try_send_message(chat_id=chat_id, 
                                                text=FIRST_MESSAGE_GROUP)
    ...
    def create_account(self, message):
        try:
            self.data_base.add_user(message.from_user.id , 
                                message.from_user.username)
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True,
                                            row_width=2)
            item1 = types.KeyboardButton("Set time zone")
            markup.add(item1)
            self.try_send_message(message.from_user.id, NEED_TIME_ZONE_TEXT(), 
                                reply_markup=markup)
            self.data_base_statistic.insert_new_user(message.from_user.id, 
                                                    message.from_user.username)
    ...
    def set_time_zone(self, message):
        try:
            ...
            for i in range(1, 8):
                item = types.KeyboardButton(f"Set time zone +{i} UTC")
                markup.add(item)
            self.try_send_message(message.from_user.id, 
                                CHOOSE_TIME_ZONE_TEXT(), reply_markup=markup)
    ...
    def delete_account(self, message):
        ...
        try:
            ...
            self.try_send_message(message.from_user.id, DELETE_ACCOUNT_TEXT(),
                                reply_markup=markup)
    ...
    def sure(self, message):
        try:
            ...
            markup.add(item1)
            self.try_send_message(user_id, DELETED_ACCOUNT_TEXT(), 
                                reply_markup=markup)
        except Exception as e:
    ...
    def stop_searching(self, message):
        try:
            checker = self.data_base.get_user_info_from_id(message.
                from_user.id)
            if checker[7] == "False":
                self.try_send_message(message.from_user.id, 
                                    ALREADY_STOP_SEARCHING_TEXT())
                return
            self.data_base.change_active_status(message.from_user.id, "False")
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, 
                                            row_width=2).add(types.
                                                        KeyboardButton("Menu"))
            self.try_send_message(message.from_user.id, STOP_SEARCHING_TEXT(),
                                reply_markup=markup)
    ...
    def set_time_zone_func(self, message, match):
        ...
        self.try_send_message(message.from_user.id, f"""Done, your time zone:
                            {time_zone} hour/s """, reply_markup=markup)

    def set_active_time_panel(self, call):
        self.choose_time[call.from_user.id] = 'none'
        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("See time options", 
                                        callback_data="show_time_panel")
        markup.add(item1)
        self.start_message_id[call.from_user.id] = self.try_send_message(
            call.chat.id, INSTRUCTION_FOR_SET_ACTIVE_TIME(), 
    ...
    def show_time_panel(self, call):
        ...
        for i in range(24):
            ...
            column_1.append(types.InlineKeyboardButton(f"{time}", 
                                callback_data=f"starttime_{time}"))
            column_2.append(types.InlineKeyboardButton(f"{time}", 
                                callback_data=f"endtime_{time}"))
        ...
        self.bot.edit_message_text(chat_id=call.message.chat.id, 
            message_id=self.start_message_id[call.from_user.id], 
            text=f"{INSTRUCTION_FOR_SET_ACTIVE_TIME()}{START_TIME}",
            reply_markup=markup)


    def hide(self, call):
        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("Got it!",
                                        callback_data="show_time_panel")
        markup.add(item1)
        self.bot.edit_message_text(chat_id=call.message.chat.id, 
            message_id=self.start_message_id[call.from_user.id],
            text=INSTRUCTION_FOR_SET_ACTIVE_TIME(), reply_markup=markup)
    ...
    def get_users_time(self, user_UTC_time, time_str):
        ...
        new_time_obj = (dt.datetime.combine(dt.date.today(), time_obj) + 
                        time_delta * (-1 if user_UTC_time > 0 else 1)).time()
        now = dt.datetime.now()
        today = now.replace(hour=new_time_obj.hour, minute=new_time_obj.minute,
                            second=new_time_obj.second, microsecond=0)
    ...
    def start_time(self, call, time):
        ...
        user_UTC_time = int(self.data_base.get_user_info_from_id(
                                                        call.from_user.id)[3])
        new_time_obj = self.get_users_time(user_UTC_time ,time)
        self.try_send_message(call.from_user.id, f"Your start time: {time}", 
                                reply_markup=markup)
    ...
    def end_time(self, call, time):
        ...
        user_UTC_time = int(self.data_base.get_user_info_from_id(
                                            call.from_user.id)[3])
        new_time_obj = self.get_users_time(user_UTC_time ,time)
        self.try_send_message(call.from_user.id, f"Your end time: {time}",
                                                        reply_markup=markup)
    ...
    def start_search(self, message):
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, 
                                row_width=2).add(types.KeyboardButton("Menu"))
        person_info = self.data_base.get_user_info_from_id(
                                                        message.from_user.id)
        self.data_base_statistic.user_push_start(message.from_user.id, 
                                                message.from_user.username)
        if person_info[5] == None or person_info[6] == None:
            self.set_active_time_text(message, markup)
            return
        time_start_person = dt.datetime.strptime(person_info[5],
                                                '%Y-%m-%d %H:%M')
        time_end_person = dt.datetime.strptime(person_info[6],
                                            '%Y-%m-%d %H:%M')
        if time_start_person >= time_end_person:
            self.try_send_message(message.from_user.id, INCORRECT__TIME_TEXT(),
                                reply_markup=markup)
            return
        if self.data_base.loock_user_into_chats(message.from_user.id):
            self.already_in_group(message, markup=markup)
            return
        self.try_send_message(message.from_user.id, START_ACTIVE_TIME_TEXT(),
                            reply_markup=markup)
        self.data_base.change_active_status(message.from_user.id, "True")
        time.sleep(1)
        if len(self.data_base.get_match(time_start_person.strftime(
        '%Y-%m-%d %H:%M'), time_end_person.strftime('%Y-%m-%d %H:%M'))) > 1:
            active_users = self.data_base.get_active_users(time_start_person.
                                strftime('%Y-%m-%d %H:%M'),
                                time_end_person.strftime('%Y-%m-%d %H:%M'))
            chat_id, name_room, _, _ = self.data_base.get_free_room_id(
                                time_start_person.strftime('%Y-%m-%d %H:%M'),
                                time_end_person.strftime('%Y-%m-%d %H:%M'))
            if chat_id == None:
            ...
            link = self.bot.create_chat_invite_link(chat_id = chat_id, 
                    name=name_room, expire_date= dt.datetime.now()+
                    dt.timedelta(minutes=30))
            self.send_links_to_users(active_users, link, markup)
        else:
            time.sleep(1)
            self.try_send_message(message.from_user.id, 
                                DONT_FOUND_MATCH_TEXT(), reply_markup=markup)
        
    #Send data base 
    def get_bd(self, message, db_name:str, table_name:str=None,
                                                            columns:list=None):
        try:
            if table_name != None:
    ...
    def join_request(self, update: types.ChatJoinRequest):
        ...
        self.data_base_statistic.user_came(update.from_user.id, 
            update.from_user.username, update.chat.id)
        ...
        welcome_message = self.try_send_message(update.chat.id, 
            WELCOME_MESSAGE())
        time.sleep(10)
    ...
    def left_chat_member(self, message):
            ...
            self.dell_message(chat_id=message.chat.id,
    ...
    def send_start(self, message):
        try:
            ...
            if cheker == 1:
                ...
                message_txt = "Welcome!"
            markup.add(item1)
            self.try_send_message(message.from_user.id, message_txt, 
                reply_markup=markup)
    ...
    def text_holder(self, message):
        ...
        if (message.text not in ["Info", "Create account", "Set time zone"] and
                self.data_base.get_one_user(message.from_user.id) is None):
            markup = self.have_not_account()
            self.try_send_message(message.from_user.id, HAVE_NO_ACCOUNT_TEXT(),
                                reply_markup=markup)
        elif message.text == "Menu":
            markup = self.menu(message)
            self.try_send_message(message.from_user.id, CHOOSE_MOUTION_TEXT(),
                                reply_markup=markup)
            ...
            markup = self.menu(message)
            self.try_send_message(message.from_user.id, INFO_TEXT(),
                                reply_markup=markup)
            ...
            self.get_bd(message, 'Statistic.db', "user_came",
                ["id", "user_id", "user_name", "chat_id", "came_time"])
    ...
    def approve_weit_time(self, update):
        ...
        if update.from_user.id not in self.users_hello:
            ...
            self.bot.delete_message(update.chat.id, 
                self.approve_message.message_id)
            return
    ...
    def start(self):
        ...
        def join_request_handler(update: types.ChatJoinRequest):
            ...
            self.check_event()
            threading.Thread(target=self.approve_weit_time, 
                args=(update,)).start()

        @self.bot.message_handler(commands=['approve'])
        def aprove_func(call):
            self.users_hello.append(call.from_user.id)
            self.bot.delete_message(call.chat.id,
                self.approve_message.message_id)
            threading.Thread(target=self.join_request, 
                args=(call,)).start()
        ...
        def left_chat_member_handler(update: types.ChatMemberLeft):
            self.check_event()
            threading.Thread(target=self.left_chat_member, 
                args=(update,)).start()
        ...
        def add_chat_into_active_handler(update: types.ChatJoinRequest):
            self.check_event()
            threading.Thread(target=self.add_chat_into_active,
                args=(update,)).start()
        ...
        def delete_chat_from_active_handler(update: types.ChatJoinRequest):
            self.check_event()
            threading.Thread(target=self.delete_chat_from_active, 
                args=(update,)).start()
        ...
        def handle_callback_query_handler(call: types.CallbackQuery):
            self.check_event()
            threading.Thread(target=self.handle_callback_query, 
                args=(call,)).start()
