...
data_base = BdHelper('AsyaApp.db')
data_base_statistic = BdHelper('Statistic.db')

def upgrade_room_info_time(id_chat):
    min_time, max_time = data_base.get_time_active_chat_users(id_chat)
    data_base.update_room_info_time(id_chat, min_time, 'min_start_time')
    data_base.update_room_info_time(id_chat, max_time, 'max_end_time')
    return None
...
def create_account(message):
    try:
        ...
        data_base_statistic.insert_new_user(message.from_user.id, message.from_user.username)
...
def start_search(message):
    ...
    data_base_statistic.user_push_start(message.from_user.id, message.from_user.username)
    ...
    if time_start_person >= time_end_person:
        ...
        return
    if data_base.loock_user_into_chats(message.from_user.id):
    ...
    time.sleep(1)
    if len(data_base.get_match(time_start_person, time_end_person)) > 1:
        active_users = data_base.get_active_users(time_start_person, time_end_person)
        chat_id, name_room, _, _ = data_base.get_free_room_id(time_start_person, time_end_person)
        if chat_id == None:
...
def join_request(update: types.ChatJoinRequest):
    ...
    data_base_statistic.user_came(update.from_user.id, update.from_user.username, update.chat.id)
...
if __name__ == '__main__':
    while True:
        try:
            time_cheker = TimeCheker(database=data_base, bot=bot, write_logs=write_logs)
